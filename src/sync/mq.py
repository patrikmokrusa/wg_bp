...
class MessageQueueSync(SyncBase):
    ...
    def __init__(self, state: State, seed_node: dict | None = None, port: int = 5555, interval: float = 0.1) -> None:
        ...
        """ Local MQ state of peers. """
        self.version = 0
        """ Version of local MQ state. """
    ...
    def getInfo(self) -> dict:
        ...
        info = {
            "sync-type": "MQ",
            "sync-seed": self.peers[self._state.ip],
        }
        return info
    ...
    def checkForChanges(self) -> None:
        ...
        # print(f"[MQ] Checking for changes in peers {self.peers.keys()} vs state peers {self._state.peers.keys()}...")
